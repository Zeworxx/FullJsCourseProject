...
import { topicsFormatValues } from '../utils/topic-format';
...
export class TodolistJSONService implements TodolistServices {
    ...
    async getTopics(userId: number): Promise<TopicContrat[]> {
        try {
            ...
            const topicsList: TopicContrat[] = (await this.dbService.pg.query(sql, [userId])).rows.map(row => { return topicsFormatValues(row) });
            return topicsList
        } catch (e) {
            console.error(e)
            throw e
